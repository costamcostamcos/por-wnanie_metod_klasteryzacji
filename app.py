...
def wczytaj_i_przygotuj_dane(plik_excel, pomin_pierwsza_kolumne):
    """Wczytuje i ewentualnie odrzuca pierwszą kolumnę."""
    df = pd.read_excel(plik_excel)
    
    if pomin_pierwsza_kolumne:
        # Bierzemy wszystkie wiersze, i kolumny od drugiej do końca
        X = df.iloc[:, 1:].values
    else:
        X = df.values
...
def generuj_wykres_srednich(X, etykiety, nazwa_algorytmu, limit_y=None):
    """Generuje wykres z opcjonalnym limitem osi Y."""
    ...
    # --- NOWE: Ograniczenie osi Y ---
    if limit_y > 0.0:
        # Ustalamy dolną granicę wykresu na najmniejszą wartość w danych (z lekkim marginesem)
        min_y = np.min(X)
...
pomin_kolumne = st.sidebar.checkbox("Zignoruj pierwszą kolumnę (np. jeśli to numer próbki lub ID)", value=False)
limit_osi_y = st.sidebar.number_input("Maksymalna wartość osi Y na wykresie (wpisz 0 aby wyłączyć limit):", min_value=0.0, max_value=1000.0, value=0.25, step=0.05)
...
if wgrany_plik is not None:
    try:
        with st.spinner('Wczytywanie i skalowanie danych...'):
            X, X_scaled, oryginalny_df = wczytaj_i_przygotuj_dane(wgrany_plik, pomin_kolumne)
        ...
        if st.button("Uruchom Klasteryzację i Rysuj Wykresy", type="primary"):
            with st.spinner('Trwa obliczanie klastrów i generowanie wykresów...'):
                ...
                for nazwa_algorytmu, etykiety in wyniki.items():
                    ...
                    # Rysowanie z uwzględnieniem limitu z panelu bocznego
